...
public class PressShapedRecipe implements IShapedRecipe<CraftingContainer>, PressRecipe {
    ...
    public PressShapedRecipe(ResourceLocation resourceLocation, String group, int width, int height, NonNullList<Ingredient> ingredients, ItemStack itemStack) {
    ...
    public RecipeSerializer<?> getSerializer() {
        return ModRecipes.PRESS_SHAPED_SERIALIZER.get();
    }
    ...
    public boolean matches(CraftingContainer p_44176_, Level p_44177_) {
        System.out.println("matches test 2");
        ...
        for (int i = 0; i < p_44171_.getWidth(); ++i) {
            for (int j = 0; j < p_44171_.getHeight(); ++j) {
                ...
                for (ItemStack item : ingredient.getItems()) {
                    System.out.println("ingredient  =" + item.getItem().getRegistryName());
                }
    ...
    static NonNullList<Ingredient> dissolvePattern(String pRecipePattern, Map<String, Ingredient> p_44204_, int p_44205_) {
        NonNullList<Ingredient> nonnulllist = NonNullList.withSize(p_44205_, Ingredient.EMPTY);
        Set<String> set = Sets.newHashSet(p_44204_.keySet());
        set.remove(" ");

        for (int j = 0; j < pRecipePattern.length(); ++j) {
            String s = pRecipePattern.substring(j, j + 1);
    ...
    public static class Serializer extends net.minecraftforge.registries.ForgeRegistryEntry<RecipeSerializer<?>> implements RecipeSerializer<PressShapedRecipe> {
        private static final ResourceLocation NAME = new ResourceLocation("gsmod", "press");

        public PressShapedRecipe fromJson(ResourceLocation p_44236_, JsonObject p_44237_) {
            String s = GsonHelper.getAsString(p_44237_, "group", "");
            Map<String, Ingredient> map = PressShapedRecipe.keyFromJson(GsonHelper.getAsJsonObject(p_44237_, "key"));
            String nPattern = GsonHelper.getAsString(p_44237_, "pattern");
            int i = nPattern.length();
            System.out.println("astler pattern = " + nPattern + " size = " + i);
            NonNullList<Ingredient> nonnulllist = PressShapedRecipe.dissolvePattern(nPattern, map, i);
            ItemStack itemstack = PressShapedRecipe.itemStackFromJson(GsonHelper.getAsJsonObject(p_44237_, "result"));
            return new PressShapedRecipe(p_44236_, s, i, 1, nonnulllist, itemstack);
        }

        public PressShapedRecipe fromNetwork(ResourceLocation p_44239_, FriendlyByteBuf p_44240_) {
            ...
            return new PressShapedRecipe(p_44239_, s, i, j, nonnulllist, itemstack);
        }

        public void toNetwork(FriendlyByteBuf p_44227_, PressShapedRecipe p_44228_) {
